...
Listeners.prototype._createIrcListener = function(typeName) {
  ...
  var ircListener = function() {
    ...
    this.listenersByType[typeName].forEach(function(info) {
      ...
      // TODO pattern? or via prefix filter or something?!
      // TODO filters
      info.fn(context);
    });
  }.bind(this);
...
Listeners.prototype.add = function(types, filters, data, fn) {
  if (arguments.length === 2) {
    // addListener(types, fn)
    fn = filters;
    filters = undefined;
  } else if (arguments.length === 3) {
    if (_.isArray(filters) || _.isString(filters)) {
      // addListener(types, filters, fn)
      fn = data;
      data = undefined;
    } else if (_.isPlainObject(filters)) {
      // addListener(types, data, fn)
      fn = data;
      data = filters;
      filters = undefined;
    } else {
    ...
    // addListener(types, filters, data, fn)
    // nothing to do here
    // TODO except maybe switch filters and data for convenience when passed swapped?
  } else {
    // TODO throw/warn?
  }

  types = Utils.ensureArray(types);
  filters = Utils.ensureArray(filters);
